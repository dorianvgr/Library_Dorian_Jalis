...
void login(FILE *data_base_users,FILE *admin_file){
    ...
    if(answer==2){ // if he types 2, that means he wants to create an account
        ...
        char S_or_P = fgetc(data_base_users);
        ...
        for (int i=0; i<nbbooks; i++){
            ...
            if (books[i].borrow==variable_for_id_check){ // looks if a book is already borrowed by the user
                ...
                for(int j=0;j<strlen(books[i].title);j++){ // ?? c pas strlen(books[i].title) ?
    	...
    	int answer_second_interface = second_interface(); // displays the second interface
        ...
        if (answer_second_interface==1){ // he wants to borrow a book
            ...
            if (answer_third_interface!=4){
                ...
                Book books_title_author_or_category[nbbooks]; // create another table to store the books the user wants to display
        ...
        // if (answer_second_interface==2){
        //     return_book(books, books_borrow,nb_books_borrow, variable_for_id_check,nbbooks);
        // }
